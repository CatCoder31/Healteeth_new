...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE user SET full_name = ?, email_address = ?, contact_number = ?, full_address = ?, gender = ?, language = ?, emergency_contact_name = ?, emergency_contact_number = ?, profile_photo = ? WHERE id = ?";

    if ($stmt = mysqli_prepare($con, $sql)) {
        // Bind variables to the prepared statement as parameters
        mysqli_stmt_bind_param($stmt, "sssssssssi", $full_name, $email_address, $contact_number, $full_address, $gender, $language, $emergency_contact_name, $emergency_contact_number, $profile_photo, $user_id);
