...
if (isset($_POST['action'])) {
    ...
    if ($action == 'Submit') {
        ...
        $update_data = "UPDATE user SET email_address=?, contact_number=?, full_address=?, gender=?, birthdate=?, emergency_contact=?, medical_history=? WHERE id = ?";
        $stmt = $con->prepare($update_data);
        $stmt->bind_param('ssssssssi', $emailupdt, $numupdt, $addupdt, $genderupdt, $dayhupdt, $emercontupdt, $medhisupdt, $id);
